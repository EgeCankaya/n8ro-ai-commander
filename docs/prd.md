...
| **C9** | The cache-minimum guard's **runtime** half, and AIC-BE-2's unmet `SHALL` | One `LlmResult` field, and a PRD revision before the code. **C4 is the same seam** |
| **C4** | `cache_creation_input_tokens` captured by neither `ILlmClient` nor the harness | The same seam as C9; quantified at **under 0.1 %** of cost |
...
**The runtime half is deliberately absent and is carried as C9, not omitted.** It cannot be written against `prefix()` — AIC-BE-3 v1.8.4(a) establishes that reading the prefix alone is wrong by 69 % about the quantity it guards, and after C3 the prefix text is ~2,212 tokens against a 4,096 minimum, so a `prefix()`-based check would emit a false shortfall warning on **every** run. It has to compare `usage.cache_read_input_tokens`, which is **5,118 tokens per hit** and clears the minimum by 1,022 (24.9 %). That value does not reach the sim thread: `LlmResult` carries no cache field, so AIC-BE-2's own recording `SHALL` is unmet as well (§Corrections item 33(f)). **One missing field, both halves, one carried item.**

**And it is not a *startup* comparison in any case.** The first request of a run is always a cache **write** and reads zero — 119 of 120 in C3's arms, 95 of 96 in C2's. A check that fires at startup has nothing to read; the earliest honest moment is the second response of a run.
