...
**Date:** 2026-07-31 (revised 2026-08-04)
**Status:** Draft v1.8.3
**Revision history:**
...
*(v1.8.2 — the target is **met**, on a measurement rather than an estimate, and the sentence above was computed against a prefix that was never deployed.)* The deployed prefix measures **4,489 tokens** on Anthropic's tokenizer (§Corrections item 21), which clears Haiku 4.5's 4,096-token cache minimum. The applicable figure is therefore the **cached** one: **$0.76 per four-ship-hour against the ≤ $1.10 target — MET, with 31 % headroom.** The uncached figure is **$3.66**, which misses by 3.3×. Both numbers are real and the product does not currently tell an operator which of the two they are paying; that gap, and the 9.6 % margin the cached figure depends on, are §Corrections item 21. The target stayed unchanged across every revision that could not meet it, which is why it is worth something now that it can.
...
**Assumptions, and which of them are now measurements.** *(v1.8.2 — the prefix is measured; the other two inputs are not, and the table below marks the difference because this section's entire output is arithmetic over these three numbers.)*

| Input | Value | Status |
|---|---|---|
| Stable prefix | **4,489 tokens** | **MEASURED** — `POST /v1/messages/count_tokens`, `claude-haiku-4-5`, 2026-08-04, against the 17,756-byte prefix dumped from the deployed `PromptRenderer` (doctrine 6,932 B, schema 8,951 B). **3.955 bytes/token** |
| Volatile suffix | ~200 tokens | **ASSUMED** — carried unmeasured since v1.2. Resolvable from `usage.input_tokens` minus the prefix count on any live order |
| Output | ~80 tokens | **ASSUMED** — carried unmeasured since v1.2. Resolvable from `usage.output_tokens` |
| Cadence | 20 s → 180 orders/entity-hour, 720/four-ship-hour | Configuration, not an estimate |
| Budget | **$5 in loaded credit** | Corrected v1.8.2 — this section said **$100** from v1.2 through v1.8.1 and the actual balance is $5. Every "hours on $100" figure this document ever printed was 20× the reality |

**Every uncached figure below is 2.8× the v1.3 table**, because the prefix the v1.3 table was computed against (1,200 tokens) was the doctrine-less one §Corrections item 16 established was never the shipping prefix. Nothing else in the model changed.

| Model | $/MTok in | $/MTok out | $/order **uncached** | $/four-ship-hour **uncached** | $/order **cached** | $/four-ship-hour **cached** | Four-ship hours on $5 (cached) |
|---|---|---|---|---|---|---|---|
| `claude-haiku-4-5` | $1 | $5 | $0.00509 | **$3.66** | $0.00105 | **$0.76** | **≈ 6.6** |
| `claude-sonnet-5` (introductory, through 2026-08-31) | $2 | $10 | $0.01018 | $7.33 | $0.00210 | $1.51 | ≈ 3.3 |
| `claude-sonnet-5` (list) | $3 | $15 | $0.01527 | $10.99 | $0.00315 | $2.27 | ≈ 2.2 |
| `claude-opus-5` | $5 | $25 | $0.02545 | $18.32 | $0.00524 | $3.78 | ≈ 1.3 |

*(Prior values for comparison — v1.2: Haiku $0.00140/order, $1.01/four-ship-hour. v1.3: $0.00180, $1.30. Both were computed against a prefix that was never deployed.)*

The non-Haiku rows scale the **same measured token count** by each model's published rate. That assumes tokenization is identical across current Claude models; `count_tokens` takes a `model` parameter, so it is an assumption this document has not tested and would cost one request to remove.

**Is the ≤ $1.10 target met? Yes — but only because the prefix caches, and it caches by 9.6 %.**

The measurement resolves OQ-8 in the direction v1.7.1 predicted: 4,489 tokens **clears** Haiku 4.5's 4,096-token prompt-cache minimum, so the prefix caches **as written** and the padding delta is zero. Nobody has to write filler doctrine to earn a discount.

| Regime | $/four-ship-hour | vs ≤ $1.10 target |
|---|---|---|
| Cached (applicable — the prefix clears the minimum) | **$0.76** | **MET**, with 31 % headroom |
| Uncached (what a cache miss costs) | $3.66 | **MISSED by 3.3×** |

**The two regimes differ by 4.8×, and nothing in the product surface tells an operator which one they are in.** That is the finding this measurement produced beyond the number itself, and it has two parts:

- **The margin is 393 tokens.** Deleting roughly **1,554 bytes** of `data/doctrine.txt` — a page — drops the prefix under the minimum. There is no error, no warning at the API, and no counter; the only symptom is the invoice quadrupling. AIC-BE-3's startup comparison is therefore not a formality that will now stop firing: it is the *only* guard on a 4.8× cost cliff, and §Corrections item 21 records that its threshold is now a live boundary rather than a distant one.
- **A cache write costs $0.0056** and recurs when the 5-minute TTL lapses. A 20 s cadence never allows that mid-run, so it is negligible in the steady state — but it is not zero for short runs, and the cached row above **excludes it**, which is honest only for runs long enough to amortize it. A 12-order run pays it on order one and reads it 11 times; a run of one order pays 1.25× and reads nothing.

**Recommendation:** default `claude.model = claude-haiku-4-5`, unchanged — and now on a measured basis rather than an assumed one. At **$0.76 per four-ship scenario-hour cached**, the $5 balance funds roughly **6.6 hours** of live four-ship operation; at the uncached $3.66 it funds **1.4**. That is enough for the Phase 2 gate (~240 orders ≈ $0.25 cached) and is *not* enough for extended evaluation, which is a budget fact this section previously hid behind a $100 figure that was never true. Reserve Sonnet 5 for order-quality comparison runs and keep Opus 5 out of the control loop entirely; at 5× Haiku's cost for a decision that emits six posture values, it is the wrong instrument.
