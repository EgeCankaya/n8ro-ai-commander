...
| # | Item | Blocked on |
|---|---|---|
| **C13** | **An `engage` order named an inbound munition as its target and nothing rejected it** — two of three commanded launches on the first clean run went at `BlueF16_01_wpn_32300_6` *(new, v1.8.22, §Corrections item 38(c))* | A design decision across three layers: a Stage-B target-class check, a doctrine line, or a filter on what Tier 1 reports. **Costs nothing, needs no grant.** Unobservable before v1.8.21 — no commanded aircraft had ever fired |
