...
**Date:** 2026-07-31 (revised 2026-08-06)
**Status:** Draft v1.8.29
**Revision history:**
...
|---|---|---|---|---|
| **200-order soak** | The full pipeline against the six **synthetic `LiveMain` fixtures** — invented values in a real snapshot's field set | 200 (local), 240 (hosted) | **100 %** | **Yes — ≥ 95 %, and the row above reads MET** |
| **Paired live runs** | The same pipeline inside the engine against **real scenario state** from a deployed mission | **54 resolved**, 7 pairs, 2 models | **~61 % pooled** (33 of 54) | **No — reported only** |
...
| **C14** | **`reject.shape` is running at 12.5 % on the local backend after C3, against a pre-C3 baseline of 0 of 200** — four occurrences, all the same conditional rule (`posture 'hold' requires orbitRadiusM > 0`) *(new, v1.8.23, §Corrections item 39(d))* | A decision on whether C3's prefix reduction is safe for the **local** backend. **Costs nothing, needs no grant.** C3 was decided on hosted evidence only — 120/120, 0 % — and no local run happened between C3 and 2026-08-06 |
