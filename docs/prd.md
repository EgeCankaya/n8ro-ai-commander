...
| C10 | **Haiku 4.5's fixed term — time to response headers — has never been measured**, and §Success metrics' ≤ 2.5 s row is Haiku's. *(Opened v1.8.16, §Corrections item 33(d).)* | Not a Phase 3 defect: C2's TTFT run was designed against `claude-sonnet-5` for the reason recorded in the fifth grant, and it answered the question it was built for. It became a scoping error only when its conclusion was written across models, which items 24 and 28 both argue against | **Answerable for ≈$0.10** — the archived request bodies plus `measure-ttft.ps1`, on `--claude-model claude-haiku-4-5`. **Not requested, and no grant is sought here.** The latency row reads **MISSED** on either result; what the measurement would settle is whether the *cause* C2 closes on is Haiku's cause or only Sonnet's |
...
The phase **entered** at $0.40 spent and **leaves at ≈$2.50 of $5**. §Cost model's inputs table carried `$0.40 spent` throughout — corrected in this revision, §Corrections item 34(b).
...
| **C10** | Haiku 4.5's fixed term — time to response headers — is unmeasured | A grant. **≈$0.10, not requested.** The latency row reads MISSED either way |
