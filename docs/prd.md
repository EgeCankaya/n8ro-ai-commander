...
| C7 | **`claude.maxTokens = 512` is Haiku-shaped and truncates Sonnet** (§Corrections items 24(b), 25). *(Opened v1.8.5, narrowed v1.8.6.)* | Not a Phase 2 defect: 512 was chosen against the only model then measured, and Haiku uses 25 % of it at its worst over 120 orders. It became wrong when a second model was run, not when it was set | **Blocked on a measurement that has not been authorized, and deliberately not guessed.** The output-length distribution is right-censored at 512, so the run that found the problem cannot size the fix. Raising it is a **config-surface change requiring a PRD revision first**, and the value requires a re-run with the ceiling raised. Both are owner decisions |
