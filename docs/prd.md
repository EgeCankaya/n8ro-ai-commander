...
| # | Item | Blocked on |
|---|---|---|
| **C16** | **The `reason` field's 1–200 character cap discards whole orders from a more verbose model** — twice at exactly 204 characters *(new, v1.8.24, §Corrections item 40(c))* | A decision, not a bug: `reason` carries no control authority, yet an over-long one costs the posture, target and waypoint with it. **Costs nothing, needs no grant** |
| **C15** | **`qwen2.5:14b` emits `cruiseSpeedMps` of 0 in 4 of 4 runs, always on `RedSu35_02`** — reproducible, and absent from every 7B run *(new, v1.8.24, §Corrections item 40(c))* | Diagnosis. Perfectly reproducible, so it is cheap to investigate. **Costs nothing, needs no grant.** Only binds if a 14B is ever adopted |
