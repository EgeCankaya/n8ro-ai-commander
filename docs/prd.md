...
| C2 | **p95 latency is 4,615 ms against a ≤ 2.5 s target** — missed by 85 %, with p50 (2,602 ms) also over. **(v1.8.5 — decomposition run, INCONCLUSIVE. v1.8.9 — that verdict is now suspect; see §Corrections item 27(f).)** | The target was set in v1.2 with no hosted measurement behind it, and this is the first real sample. Nothing was tuned to chase it. The 20 s cadence absorbs p99 (7,099 ms), so no run was degraded — it is a target failure, not a control-loop failure | Phase 3, **reopened**. The C7 run supplied the control the C2 arms lacked: latency-vs-output r² is **0.004/0.071 on Haiku but 0.574/0.734 on Sonnet**, whose output range is 5× wider. **Restricted range mechanically suppresses r²**, so "INCONCLUSIVE" was concluded from arms with a flattened x-axis. The TTFT instrument is still the right next step — now to separate the fixed term from generation on a run where generation is known to be **57 %** of it |
