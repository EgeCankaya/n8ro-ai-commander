...
Non-Haiku rows scale the measured token counts by published rates. Tokenization is assumed identical across current Claude models; only Haiku was measured.
...
| C2 | **p95 latency is 4,615 ms against a ≤ 2.5 s target** — missed by 85 %, with p50 (2,602 ms) also over | The target was set in v1.2 with no hosted measurement behind it, and this is the first real sample. Nothing was tuned to chase it. The 20 s cadence absorbs p99 (7,099 ms), so no run was degraded — it is a target failure, not a control-loop failure | Phase 3: decide whether the target was wrong or the path is slow. Neither is established. Candidate contributors — 7,608 cached input tokens, `max_tokens` 512, no streaming — are all unmeasured, and picking one without measuring is how §Corrections gets longer |
| C3 | **The order schema is transmitted twice per request**, ~71 % of the cached prefix, ~52 % of the bill (§Corrections item 22) | It is a design consequence, not a bug: the prose rendering predates structured outputs, when `format` was the only constraint. Removing it is an optimization whose effect on order quality has never been measured | Phase 3, and it is the highest-value cost lever identified: ~$0.89/four-ship-hour versus today's $1.05, with the prefix still cache-eligible |
