...
| `claude.maxTokens` | Int | `512` | **Model-shaped, like `claude.effort` below it.** Range `1 … 8192`. The default is sized for `claude.model`'s default: over 120 orders Haiku 4.5 peaks at **127 tokens — 4.0× headroom**. It is **not** safe for a more verbose model: Sonnet 5 truncated **4 of 48** at this value with **1.02×** headroom on its largest completed order, and the truncation surfaces as `parse`/`envelope`, never as a length error (§Corrections items 24(b), 25). **Raising it for a different model requires measuring that model's output distribution first** — the 512-run's data is right-censored at 512 and cannot size its own replacement. The upper bound is derived, not chosen: 8,192 keeps a full-length response under Stage A's 64 KiB `kMaxResponseBodyBytes` and inside the non-streaming completion band (§Corrections item 26) |
...
|---|---|---|---|
| 48 orders, 2026-08-04 | **$0.00784** | **$5.65** | scaled row said $0.00439 / $3.16 — **+79 %** |
...
|---|---|---|---|
| C1 | **The in-engine live-scenario run against `oppint_red_interceptor` was not performed.** `run-live-scenario.ps1` is untouched on the hosted backend | **Not a defect, an authorization boundary.** The v1.8.3 grant covers the synthetic `LiveMain` fixtures and explicitly not real scenario state from a deployed mission file. The distinction was drawn deliberately and the run was not made rather than the grant being read generously | Needs its own owner grant. Until then the hosted backend is characterised on fixtures, and this document says so wherever it reports a hosted number |
| C2 | **p95 latency is 4,615 ms against a ≤ 2.5 s target** — missed by 85 %, with p50 (2,602 ms) also over. **(v1.8.5 — the decomposition was run and is INCONCLUSIVE; see below.)** | The target was set in v1.2 with no hosted measurement behind it, and this is the first real sample. Nothing was tuned to chase it. The 20 s cadence absorbs p99 (7,099 ms), so no run was degraded — it is a target failure, not a control-loop failure | Phase 3, still open. **The two candidate causes have now been tested against each other and neither survives** — the next step is a better instrument, not a fix |
| C3 | **The order schema is transmitted twice per request. (v1.8.5 — the cost half is now confirmed by measurement; the quality half is not.)** | Cutting the prefix by 1,617 cached tokens moved cost from **$1.05 to $0.92 per four-ship-hour (−12 %)**, confirming the mechanism. But that arm also accepted **59/60** where the full-doctrine arm accepted 60/60 | Phase 3. The single rejection is not significant at n=60 and is exactly the quality cost this item says must be measured before the prose copy is removed. **Still unmade** |
...
| C7 | **`claude.maxTokens = 512` is Haiku-shaped and truncates Sonnet** (§Corrections items 24(b), 25). *(Opened v1.8.5, narrowed v1.8.6, **half-closed v1.8.7**.)* | Not a Phase 2 defect: 512 was chosen against the only model then measured, and Haiku uses 25 % of it at its worst over 120 orders. It became wrong when a second model was run, not when it was set | **The config-surface half is DONE (v1.8.7); the value half is still blocked and still not guessed.** Done: the field is documented as model-shaped, gains a derived upper bound of 8,192, and §Corrections item 26 records the three unrelated ceilings this raise has to pass through — including the 4 KiB record cap that would silently truncate the evidence. Open: the output-length distribution is right-censored at 512, so **the run that found the problem cannot size the fix**. A ceiling-raised re-run needs its own owner grant; until then the default stays 512 |

##### The C2 latency decomposition — run, and inconclusive *(v1.8.5)*
...
**Neither hypothesis survives, and the reason is in the r² column.** Output-token count explains essentially **none** of the latency variance in either arm — 0.4 % and 7 %. The intercept/slope split the regression produces (2,454 ms + 1.8 ms/token for A, 466 ms + 20.8 ms/token for B) is therefore **fitting noise**, and the fact that two arms of the same system disagree about it by 5× is the tell. Those numbers must not be quoted as a decomposition; they are what a near-zero-r² fit does.
...
**What this establishes:** latency here is dominated by **run-to-run and within-run variance** — service-side queueing and scheduling — not by either quantity the product controls. **What it does not establish:** that the prefix is irrelevant. It establishes that its effect, if any, is smaller than the noise at n = 60, which is a statement about the instrument as much as the system.
