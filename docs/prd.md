...
| # | Item | Blocked on |
|---|---|---|
| **C18** | **`own.headingDeg` is read from a schema leaf documented as the heading at `t=0`**, on a component whose runtime motion is *"owned by the physics pass, not by this datablock"* — so the snapshot may have carried a spawn-time constant labelled as current heading in every prompt ever sent *(new, v1.8.25, §Corrections item 41(f))* | **A measurement, and the instrument does not exist yet.** `order.requested` carries `promptHash` and `snapshotHash` and no snapshot content, and the hash cannot stand in — position moves every tick, so it changes whether or not heading does. Answering it needs an **AIC-DET-1 record-format change**, which is an FR-behaviour change and goes in this document first. **Costs nothing, needs no grant** |
| **C17** | **§Success metrics reads acceptance MET at 100 % on synthetic fixtures while the engine measures ~61 % pooled over 54 resolved orders** — and only the fixture instrument is gated *(new, v1.8.25, §Success metrics, §Corrections item 41)* | **An owner decision on where the bar sits.** v1.7.5 put it on the soak because the in-engine n was 10; **it is now 54, and the interval on 33/54 — roughly [47 %, 74 %] — excludes both bars.** Moving it onto the engine makes the metric answerable only by a ~22-minute paired run; leaving it keeps a green row the product does not reproduce against real state. **Costs nothing, needs no grant** |
...
| **C20** | **The 14B arm cannot be run on this host: the model needs 15.75 GB and sits at 93 % GPU / 7 % CPU**, with the GPU at 16,002 of 16,376 MiB and ~5.8 GB held by desktop processes. Two attempted pairs returned **0 of 2 resolved, every rejection `reject.transport` at the 90 s budget** *(new, v1.8.26, §Corrections item 42(f))* | **A host condition, not a code defect** — the 7B ran three clean pairs on the same build and the same prefix at 1.8–9.5 s, and archived 14B runs on this host returned in 3.7–17.1 s. **Consequence: C16's fix is unmeasured in the engine**, because it only ever fired at 14B. Needs a quieter host or a smaller resident footprint |
| **C19** | **Nothing bounds a commanded speed from below.** Stage A's A7 checks `> 0`; Stage B's B6 checks only the upper bound; `CommanderConfig` has `minAltitudeHaeM` and **no `minSpeedMps`**. A `1.5` m/s cruise order for a fighter passes both stages, and five were accepted *(new, v1.8.26, §Corrections item 42(d))* | **An FR change — a new `safety.*` field — so it goes in this document before it is in code.** The asymmetry is already visible in the config: altitude has a floor because too low is dangerous, and the same argument applies to speed. **Costs nothing, needs no grant** |
| **C15** *(reopened)* | **The fix corrected the sign and not the selection.** Post-fix the model emits **full-precision floats copied out of the prompt** — clustering at ≈320.0, ≈318.93 and ≈1.50, the signature of choosing among `own.speedMps`, `own.velE` and `own.velN` — where pre-fix it typed round numbers. One entity swung **320 → 1.5 → 318.93 across consecutive 20 s windows** *(reopened v1.8.26, §Corrections item 42(c))* | **A decision about what the suffix should carry at all.** The suffix used to offer three numbers that look like speeds and now offers four; adding a fourth made a copied value *positive*, which is why `range` and `clamp` went to zero — **a positive wrong number is one the validator has no reason to refuse.** Remedies sit in two layers (what is transmitted, and C19's floor) and choosing between them is a design decision. **Costs nothing, needs no grant** |
