...
| `own.headingDeg` | `componentTransform` schema leaf `headingDeg` | Degrees clockwise from true north. *(v1.8.25 — this leaf is documented as the heading **at t=0**; whether it tracks the physics pass is open as **C18**)* |
...
| # | Item | Blocked on |
|---|---|---|
| **C18** *(instrument built v1.8.27; question still open)* | **`own.headingDeg` is read from a schema leaf documented as the heading at `t=0`**, on a component whose runtime motion is *"owned by the physics pass, not by this datablock"* — so the snapshot may have carried a spawn-time constant labelled as current heading in every prompt ever sent *(new, v1.8.25, §Corrections item 41(f))* | **The blocker has changed and the row has not closed, which is the distinction worth keeping.** `order.requested` now carries the own-ship block (AIC-DET-1, v1.8.27), so the question is answerable — **it has not been answered.** One run on the new build settles it, and it settles §Corrections item 42(e) with it: whether an accepted 1.5 m/s came from the model or from the snapshot. **Closing this row on "the instrument exists" would be closing it on the wrong thing.** Costs nothing, needs no grant |
